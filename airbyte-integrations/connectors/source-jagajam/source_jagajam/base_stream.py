...
class JagajamStream(HttpStream, ABC):
    ...
    def _send_request(
        ...
        user_backoff_handler = user_defined_backoff_handler(max_tries=max_tries)(
            self._send
        )
    ...
    def parse_response(
        ...
        if body.get("meta", {}).get("code") == 1000:
            logger.info(
                "Data not ready (meta.code=1000); skipping slice without retry."
            )
            return
    ...
    def should_retry(self, response: requests.Response) -> bool:
        ...
        should_retry_on_meta = (
            meta_status_code in [429, 408] or 500 <= meta_status_code < 600
        )
