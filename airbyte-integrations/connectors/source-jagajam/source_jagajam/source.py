...
class SourceJagajam(AbstractSource):
    ...
    def transform_config_date_range(config: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        if date_range_type == "custom_date":
            date_from = datetime.strptime(
                date_range.get("date_from"), from_user_date_format
            )
            date_to = datetime.strptime(
                date_range.get("date_to"), from_user_date_format
            )
    ...
    def get_available_communities(self, config):
        ...
        return list(
            communities_stream_instance.read_records(sync_mode=SyncMode.full_refresh)
        )
