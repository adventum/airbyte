    ...
    def get_json_schema(self) -> Mapping[str, Any]:
        ...
        if self.fields_to_include:
            if not self.use_included_fields:
                raise Exception(
                    f"Included fields is not supported for {self.name} stream."
                )
            ...
            for field_to_include_name in self.fields_to_include:
                ...
                included_fields_schemas[field_to_include_name] = (
                    object_schema["properties"][field_to_include_name]
                )
...
class IncrementalStatisticsMixin(MyTargetStream, IncrementalMixin, HttpSubStream, ABC):
    ...
    def request_params(
        ...
        params = {
            "date_from": stream_slice["date_from"],
            "date_to": stream_slice["date_to"],
            "id": ",".join(stream_slice["ids"]),
        }
        return params
    ...
    def get_json_schema(self) -> Mapping[str, Any]:
        ...
        ).get_schema("statistics")
        schema["properties"].update(
            {self.object_id_property_name: {"type": ["null", "string"]}}
        )
        extra_properties = ["__productName", "__clientName"]
        custom_keys = json.loads(self.config.get("custom_json", "{}")).keys()
        extra_properties.extend(custom_keys)
    ...
    def read_records(
        ...
        for record in MyTargetStream.read_records(
            ...
            if stream_slice["should_update_state_after"]:
                ...
                self._cursor_value = (
                    max(self._cursor_value, latest_record_date)
                    if self._cursor_value
                    else latest_record_date
                )
            yield record

    @staticmethod
    def date_chunks_ranges(
        date_from: str, skip_first: bool, date_to: str = None
        ...
        *,
        ...
        if sync_mode == SyncMode.incremental:
            ...
            if stream_state and self.cursor_field in stream_state:
                ...
                if (
                    datetime.strptime(stream_state[self.cursor_field], DATE_FORMAT).date()
                    > datetime.today().date() - timedelta(1)
                ):
                    stream_state[self.cursor_field] = (
                        (datetime.today().date() - timedelta(1)).strftime(DATE_FORMAT)
                    )
                else:
            ...
            # if first load:
                ...
                date_from = self.date_from
        date_ranges = self.date_chunks_ranges(
            date_from, skip_first=bool(stream_state), date_to=self.date_to
        )
        if not date_ranges:
        ...
        for date_range in date_ranges:
            for ids_chunk_num, objects_ids_chunk in enumerate(
                objects_ids_chunks, start=1
            ):
                slices.append(
                    {
                        ...
                        "should_update_state_after": ids_chunk_num
                        == len(objects_ids_chunks),
                    }
                )
        return slices
