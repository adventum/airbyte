...
class YandexDiskResource(HttpStream, ABC):
    ...
    def __init__(
        ...
        self.resources_filename_pattern = re.compile(resources_filename_pattern)
    ...
    def get_json_schema(self) -> Mapping[str, Any]:
        ...
        ).get_schema("yandex_disk_resource")
    ...
    @staticmethod
    def add_filepath_to_record(
        record: Dict[str, Any], file_path: str
    ) -> Dict[str, Any]:
        record["__filepath"] = file_path
        return record

    def apply_field_name_map(
        self, record: Dict[str, Any], field_name_map_individual: Dict[str, str]
    ) -> Dict[str, Any]:
    ...
    def parse_csv_response(
        self, response: requests.Response, file_path
    ) -> Iterable[Mapping]:
        lines_gen = (
            line.decode("utf-8").replace("\ufeff", "") for line in response.iter_lines()
        )
        ...
        for values_line_n, values_line in enumerate(lines_reader):
            if values_line_n == 0:
                if self.user_specified_fields and len(values_line) != len(
                    self.user_specified_fields
                ):
    ...
    def parse_excel_response(
        ...
        with io.BytesIO(response.content) as fh:
            ...
            for record in df.to_dict("records"):
                for record_key in list(record.keys()):
                    if record_key in self._field_name_map:
                        record[self._field_name_map[record_key]] = record.pop(
                            record_key
                        )

                record = self.apply_field_name_map(
                    record, self.field_name_map_individual
                )
                record = self.add_constants_to_record(record)
                record = self.add_filepath_to_record(record, file_path)
                yield record
    ...
    def parse_response(
        ...
        file_path = kwargs.get("file_path") or kwargs.get("stream_slice", {}).get(
            "file_path"
        )
        if self.resource_files_type == "CSV":
            ...
            raise Exception(f"Unsupported file type: {self.resource_files_type}")
    ...
    def get_path_resources(self):
        ...
        while True:
            ...
            resources_response_items = resources_response.json()["_embedded"].get(
                "items"
            )
            available_resources = [*available_resources, *resources_response_items]
    ...
    def derive_fields_names_from_sample(self):
        ...
        sample_record: Dict[str, Any] = next(
            self.parse_response(
                requests.get(
                    download_file_link,
                    headers=self.authenticator_copy.get_auth_header(),
    ...
    def generate_patterns_by_date(self) -> list[str]:
        """
        Generates a list of patterns by dates in a range if a date placeholder is used (!...!)
        """
        if not (self.date_from and self.date_to):
            return [self.resources_filename_pattern.pattern]
        date_format: str | None = SourceYandexDisk.contains_date_placeholder(
            self.resources_filename_pattern.pattern
        )
        if not date_format:
            return [self.resources_filename_pattern.pattern]
        days: int = (self.date_to - self.date_from).days + 1
        patterns: list[str] = []
        for i in range(days):
            d: datetime = self.date_from + timedelta(days=i)
            pattern = self.resources_filename_pattern.pattern.replace(
                f"!{date_format}!", d.strftime(date_format)
            )
            patterns.append(pattern)
        return patterns

    def lookup_resources_for_pattern(self):
        availabe_path_resources: list[dict[str, Any]] = self.get_path_resources()
        resources: list[dict[str, Any]] = []
        patterns: list[str] = self.generate_patterns_by_date()
        compiled_patterns: list[Pattern[AnyStr]] = [re.compile(p) for p in patterns]
        for resource in filter(lambda r: r["type"] == "file", availabe_path_resources):
            for pat in compiled_patterns:
                if pat.match(resource["name"]):
        ...
        if not resources:
            ...
            raise Exception(
                f"Resources for stream '{self.name}' with pattern(s) "
                f"{[p.pattern for p in compiled_patterns]} not found,"
        ...
        logger.info(
            f"Stream {self.name}: found resources for patterns "
            f"{[p.pattern for p in compiled_patterns]}:"
...
class SourceYandexDisk(AbstractSource):
    def check_connection(self, logger, config) -> Tuple[bool, any]:
        ...
        if stream_name_duplicates:
            return (
                False,
                f"Each stream name must be unique. Stream names duplicates: {stream_name_duplicates}",
            )

        for stream_config in config["streams"]:
            if stream_config.get("no_header", False) and not stream_config.get(
                "user_specified_fields"
            ):
                return (
                    False,
                    f'"No Header" selected for stream {stream_config["name"]},'
    ...
    def get_auth(config: Mapping[str, Any]) -> TokenAuthenticator:
        if config["credentials"]["auth_type"] == "access_token_auth":
            return TokenAuthenticator(
                token=config["credentials"]["access_token"], auth_method="OAuth"
            )
    ...
    @staticmethod
    def transform_file_path_and_files_pattern(
        stream_config: dict[str, Any], config: dict[str, Any]
    ):
        if config.get("path_placeholder"):
            ...
            if SourceYandexDisk.contains_placeholder(files_pattern):
                updated_files_pattern = files_pattern.replace(
            ...
            if SourceYandexDisk.contains_placeholder(files_path):
                updated_files_path = files_path.replace(
                    "{placeholder}", path_placeholder
                )
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        for stream_config in config["streams"]:
            user_specified_fields = (
                stream_config.get("user_specified_fields", "").strip().split(",")
            )
            ...
            if date_type_choiced == "custom_date":
                date_from, date_to = self.get_date_range(
                    config["date_range"]["date_from"], config["date_range"]["date_to"]
                )
            ...
            stream_config = SourceYandexDisk.transform_file_path_and_files_pattern(
            ...
            streams.append(
                YandexDiskResource(
                    ...
                    field_name_map_individual={
                        item["old_value_individual"]: item["new_value_individual"]
                        for item in stream_config.get("field_name_map_individual", [])
                    },
