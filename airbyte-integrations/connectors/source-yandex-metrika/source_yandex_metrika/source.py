...
class SourceYandexMetrika(AbstractSource):
    ...
    def preprocess_raw_stream_slice(
        ...
        is_request_on_server, request_id = (
            preprocessor.check_if_log_request_already_on_server(stream_slice)
        )
        if is_request_on_server:
            logger.info(f"Log request {request_id} already on server.")
        else:
            logger.info("Log request was not found on server, creating it.")
            if check_log_request_ability:
                is_request_available, request_ability_msg = (
                    preprocessor.check_log_request_ability(stream_slice)
                )
        ...
        if isinstance(stream_instance, YandexMetrikaRawDataStream):
            ...
            for raw_slice in raw_slices:
                ...
                for thread in threads_controller.threads:
                    ...
                    yield from (
                        self._get_message(record, stream_instance)
    ...
    def check_connection(self, logger, config) -> tuple[bool, any]:
        ...
        """Check names"""
        if len([stream.name for stream in streams]) != len(
        ...
        if isinstance(stream, YandexMetrikaRawDataStream):
            ...
            if stream.check_log_requests_ability:
                can_replicate_all_slices, message = (
                    preprocessor.check_stream_slices_ability()
                )
    ...
    def transform_config(self, raw_config: dict[str, any]) -> Mapping[str, any]:
        ...
        raw_config["prepared_date_range"] = {
            "date_from": datetime.fromisoformat(
                date_from.isoformat()
            ),  # lazy pendulum to datetime conversion
    ...
    def streams(
        ...
        for stream_config in config.get("raw_data_hits_visits_report", []):
            stream = YandexMetrikaRawDataStream(
                ...
                field_name_map=self.format_field_name_map(
                    stream_config.get("field_name_map")
                ),
            )
